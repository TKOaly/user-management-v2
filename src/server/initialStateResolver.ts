...
export const resolveInitialState = async (token: Maybe<string>, path: string, editUserId: Maybe<string>): Promise<AppProps> => {
  ...
  const editUser =
    editUserId
      .chain(id => id === 'me' ? Just(me) : Maybe.fromNullable(userList.find(u => u.id === Number(id))))
